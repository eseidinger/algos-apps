 ...
 */
 ...
 export class Rectangle {
     ...
     constructor(xMin: number, yMin: number, xMax: number, yMax: number) {
         ...
         this.lineSegments = [
             new LineSegment(this.points[0], this.points[1]),
             new LineSegment(this.points[1], this.points[2]),
             new LineSegment(this.points[2], this.points[3]),
             new LineSegment(this.points[3], this.points[0]),
         ];
     }
     ...
     public getMaximumDistanceFromBorder(point: Vector): number {
         const distances = this.points.map((p) => p.sub(point).abs());
         distances.sort(comparator.compare);
         return distances[3];
     }
